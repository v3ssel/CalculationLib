...
namespace s21 {
void Model::depositCalc(long double& amount, 
                        int64_t&     termDays,
                        bool&        rateType,
                        int64_t*     daysOrAmountRate,
                        size_t&      rateCounter,
                        size_t&      size,
    ...
    for (int64_t i = 1; i <= termDays; i++) {
        if (RepWithValues && DatesInDays && cnt < size && (i == *DatesInDays || *DatesInDays == 0)) {
            *resultSum += *RepWithValues;
            DatesInDays++, RepWithValues++;
            cnt++;
        }
        
        if ((i == *daysOrAmountRate && rateType) || (*resultSum >= *daysOrAmountRate && *interestRate != lastRate && !rateType)) {
