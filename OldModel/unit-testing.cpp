...
TEST(Calculator, SumMinus1)
{
    ...
    long double answer = model.entryPoint(exp, 0);
...
{
    std::string exp = "2cos(1) + 3sin(2) - 4tan(1) + 2acos(0.32) - 3asin(0.2) + 4atan(2) - "
                      "4ln(10) + 7log(99) / 4x";
    long double answer = model.entryPoint(exp, 10);
...
TEST(Calculator, X)
{
    ...
    long double answer = model.entryPoint(exp, 3.15);
...
//   model.startCalculationLoan(sum, term, percent, credit, overpay, fullsum);
...
//   model.startCalculationTerm(sum, monthly, percent, term, overpay,
//                                   fullsum);
...
// TEST(Deposit, One) {
//   long double amount = 340000, interestRate[] = {0, 14.3, 17, 25}, taxRate =
//   8,
...
//   short period = 1;
//   for (size_t i = 0; capitalism && i < size; i++) amount +=
//   RepWithValues[i]; model.startCalculationDeposit(
...
// TEST(Deposit, Two) {
//   long double amount = 700000, interestRate[] = {0, 25, 45, 35}, taxRate =
//   5,
