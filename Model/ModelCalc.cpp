...
bool Model::parseExpresssion_(const std::string& exp, const ld& x)
{
    ...
    if (!exp.empty()) {
        ...
        for (size_t i = 0; i < len; i++) {
            if (isNumber(exp[i])) {
                if (!i || ((!expression_.empty() && expression_.top().type == OPEN_BRACKET) || (expression_.top().type >= PLUS && expression_.top().type <= POWER))) {
                ...
                case '(':
                    LBCnt++;
                    if (!expression_.empty() && (expression_.top().type == NUMBER || expression_.top().type == X))
...
{
    if (sign < 2) {
        if (((!expression_.empty() && !(expression_.top().type >= 4 && expression_.top().type <= 9)) || !i) && exp[i + 1] != '\0') {
            if (!i || exp[i - 1] == '(')
                expression_.push({ 0, 0, NUMBER });
        ...
        if (!expression_.empty() && (!isPrevNonOp(expression_.top().type) || expression_.top().type == CLOSED_BRACKET) && exp[i + 1] != '\0') {
            if (sign == 2)
                expression_.push({ 0, 2, MULT });
            else
...
void Model::lastChecker_(size_t& LBCnt, size_t& RBCnt, size_t& func_cnt,
    bool& err_code)
{
    if (LBCnt != RBCnt || (!expression_.empty() && (expression_.top().type != CLOSED_BRACKET && expression_.top().type != NUMBER && expression_.top().type != X)) || ((func_cnt * 2) > (LBCnt + RBCnt)))
        err_code = false;
}
            ...
            if (expression_.top().type == CLOSED_BRACKET) {
                ...
                if (expression_.top().type == NUMBER || expression_.top().type == X) {
                    output.push({ expression_.top().value, expression_.top().priority,
                        expression_.top().type });
                    expression_.pop();
                } else if (expression_.top().type != NUMBER && expression_.top().type != CLOSED_BRACKET) {
